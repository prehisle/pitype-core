...
- `packages/pitype-core` 已实现 TypingSession、StatsTracker、tokenizer、Locale API、DOM 渲染/光标/输入适配器，并配备 Vitest 契约测试。
- `createTextSource`/`tokenizeText` 支持中英文混排、全角标点、禁则策略；`createDomTextRenderer` 内置中文标点禁则和可配置 Hook。
- Playwright 基线测试覆盖 `typerank3` 的关键路径，验证 core API 与 demo 之间的契约。

## 下一步关注

1. **TextSource 扩展**：提供异步/流式文本源示例，完善 locale fallback 测试。
2. **更细粒度的事件文档**：补充 TypingSession 事件表，与 `docs/04接入指南.md` 中的示例联动。
3. **Demo 覆盖面**：为主题切换、移动端滚动等高级场景新增 Playwright 用例，确保核心 API 兼容多端需求。
