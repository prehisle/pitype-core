# 📋 质量门禁增量实施计划

## 🎯 实施原则

1. **每次只改一个方面** - 降低风险
2. **先本地后CI** - 快速反馈
3. **先基础后高级** - 稳扎稳打
4. **每步都测试** - 确保可用
5. **可随时回滚** - 保持安全

---

## 📈 当前进度速览

| 步骤         | 目标                             | 状态 | 备注                                                  |
| ------------ | -------------------------------- | ---- | ----------------------------------------------------- |
| 0. 准备      | 初始化 monorepo/脚本             | ✅   | config 与脚本已就位                                   |
| 1. 代码规范  | EditorConfig、Prettier、ESLint   | ✅   | lint-staged/格式化已启用                              |
| 2. Git Hooks | Husky + lint-staged + commitlint | ✅   | pre-commit/commit-msg 在用（release commit 自动豁免） |
| 3. 增强 CI   | 覆盖率 & type-check（soft fail） | ✅   | quality-gate workflow 已执行                          |
| 4. Codecov   | 覆盖率可视化                     | ✅   | CODECOV_TOKEN 配置完成                                |
| 5. 安全扫描  | npm audit/Snyk/CodeQL            | 🟡   | 已接入，阈值待收紧                                    |
| 6. 性能监控  | Bundle + TypingSession Bench     | ✅   | performance workflow 输出 summary                     |
| 7. 自动发布  | semantic-release                 | ✅   | 需确保 `NPM_TOKEN`、`GH_TOKEN` 可用                   |

---

## 📅 实施路线图（共7步）

### ✅ 第0步：准备工作（当前）

- [x] 创建所有配置文件
- [x] 更新 package.json
- [ ] 开始增量实施

---

### 🔵 第1步：基础代码规范（最低风险）⭐ 从这里开始

**目标**: 统一代码风格，不影响现有流程

**实施内容**:

1. EditorConfig 配置
2. Prettier 格式化现有代码
3. ESLint 修复

**测试验证**:

```bash
# 1. 格式化所有代码
npm run format

# 2. 检查是否有格式问题
npm run format:check

# 3. 检查 lint 是否通过
npm run lint

# 4. 运行所有测试确保没有破坏功能
npm test
```

**回滚方案**: `git reset --hard HEAD`

**预计时间**: 10分钟

**风险**: 🟢 低 - 仅格式化，不改逻辑

---

### 🔵 第2步：Git Hooks（本地质量卡点）

**目标**: 在提交时自动检查，提前发现问题

**实施内容**:

1. 安装 husky + lint-staged
2. 配置 pre-commit hook
3. 配置 commit-msg hook

**测试验证**:

```bash
# 1. 安装依赖
npm install

# 2. 初始化 husky
npm run prepare

# 3. 检查 hooks 文件
ls -la .husky/

# 4. 测试 pre-commit（应该自动格式化）
echo "const  x=1" >> test-file.js
git add test-file.js
git commit -m "test: verify pre-commit"
# 观察是否自动格式化

# 5. 测试 commit-msg（应该被拒绝）
git commit --allow-empty -m "bad commit message"
# 应该失败

# 6. 测试正确的提交消息（应该成功）
git commit --allow-empty -m "test: valid commit message"
# 应该成功

# 7. 清理测试文件
git reset HEAD~1
rm -f test-file.js
```

**回滚方案**:

```bash
rm -rf .husky
npm uninstall husky lint-staged @commitlint/cli @commitlint/config-conventional
```

**预计时间**: 15分钟

**风险**: 🟡 中低 - 仅影响本地提交流程，可用 `--no-verify` 跳过

---

### 🔵 第3步：增强现有CI（改进不破坏）

**目标**: 在现有CI基础上增强，不引入新的门禁

**实施内容**:

1. 保留现有 `.github/workflows/ci.yml`
2. 添加覆盖率报告（不强制阈值）
3. 添加类型检查

**修改方案**: 增强现有 ci.yml 而不是替换

```yaml
# 在现有 Test 步骤后添加
- name: Generate coverage report
  run: npm run test:coverage
  continue-on-error: true # 不阻塞

- name: Type check
  run: npm run type-check
  continue-on-error: true # 不阻塞
```

**测试验证**:

```bash
# 1. 本地运行覆盖率测试
npm run test:coverage

# 2. 本地运行类型检查
npm run type-check

# 3. 创建测试分支
git checkout -b test/step3-enhanced-ci

# 4. 提交并推送
git add .github/workflows/ci.yml
git commit -m "ci: add coverage and type check"
git push -u origin test/step3-enhanced-ci

# 5. 观察 GitHub Actions 是否成功运行
# 6. 检查是否生成覆盖率报告
```

**回滚方案**:

```bash
git revert <commit-hash>
# 或恢复原 ci.yml
```

**预计时间**: 20分钟

**风险**: 🟢 低 - 使用 continue-on-error，不会阻塞合并

---

### 🔵 第4步：启用 Codecov（外部服务集成）

**目标**: 可视化覆盖率趋势

**实施内容**:

1. 注册 Codecov 账号
2. 配置 CODECOV_TOKEN
3. 修改 CI 上传覆盖率

**测试验证**:

```bash
# 1. 访问 https://codecov.io 并授权 GitHub 仓库

# 2. 复制 token 并添加到 GitHub Secrets
# Settings → Secrets → Actions → New repository secret
# Name: CODECOV_TOKEN
# Value: <your-token>

# 3. 更新 CI 配置添加上传步骤

# 4. 创建测试提交
git checkout -b test/step4-codecov
git commit --allow-empty -m "ci: add codecov integration"
git push -u origin test/step4-codecov

# 5. 检查 Codecov Dashboard 是否显示报告
# https://codecov.io/gh/<your-org>/<repo>
```

**回滚方案**:

```bash
# 删除 Codecov 上传步骤
# 撤销 GitHub Secret（如果需要）
```

**预计时间**: 15分钟

**风险**: 🟢 低 - 仅上传报告，不影响CI流程

---

### 🔵 第5步：完整质量门禁（严格模式，但可选）

**目标**: 启用所有质量检查，但先作为可选项

**实施内容**:

1. 部署 quality-gate.yml（与现有 ci.yml 并行）
2. 不设置为必需检查
3. 观察运行情况

**测试验证**:

```bash
# 1. 添加 quality-gate.yml
git checkout -b test/step5-quality-gate
git add .github/workflows/quality-gate.yml
git commit -m "ci: add comprehensive quality gate workflow"
git push -u origin test/step5-quality-gate

# 2. 创建 PR 观察所有检查是否运行
# 预期：9个检查并行运行

# 3. 检查哪些通过，哪些失败
# 4. 修复失败的检查

# 5. 运行一周，观察稳定性
```

**回滚方案**:

```bash
# 删除或禁用 quality-gate.yml
rm .github/workflows/quality-gate.yml
```

**预计时间**: 30分钟 + 观察期1周

**风险**: 🟡 中 - 新增多个检查，可能需要调整配置

---

### 🔵 第6步：安全扫描和性能监控（增值功能）

**目标**: 添加安全和性能监控

**实施内容**:

1. 添加 security-scan.yml
2. 添加 performance.yml
3. 配置 Snyk（可选）

**测试验证**:

```bash
# 1. 配置 Snyk（可选）
# 访问 https://snyk.io 注册
# 添加 SNYK_TOKEN 到 GitHub Secrets

# 2. 部署工作流
git checkout -b test/step6-security-performance
git add .github/workflows/security-scan.yml
git add .github/workflows/performance.yml
git commit -m "ci: add security scan and performance monitoring"
git push -u origin test/step6-security-performance

# 3. 观察扫描结果
# 4. 修复发现的安全问题
```

**回滚方案**:

```bash
# 删除相关工作流
rm .github/workflows/security-scan.yml
rm .github/workflows/performance.yml
```

**预计时间**: 30分钟

**风险**: 🟢 低 - 独立工作流，不影响主流程

---

### 🔵 第7步：自动发布（最后启用）

**目标**: 实现自动版本管理和发布

**实施内容**:

1. 配置 NPM_TOKEN
2. 部署 release.yml
3. 测试发布流程（先发布到测试包）

**测试验证**:

```bash
# 1. 创建 npm 测试账号或使用 scope
# 修改 package.json 中的包名为 @your-scope/pitype-core-test

# 2. 生成 npm token
# https://www.npmjs.com/settings/<username>/tokens
# 添加到 GitHub Secrets: NPM_TOKEN

# 3. 在 dev 分支测试
git checkout -b test/step7-auto-release
git add .github/workflows/release.yml .releaserc.json
git commit -m "ci: add semantic release workflow"
git push -u origin test/step7-auto-release

# 4. 合并到 main 触发发布（确保一切就绪！）

# 5. 观察是否成功发布
```

**回滚方案**:

```bash
# 禁用 release.yml
# 手动发布版本修复
```

**预计时间**: 45分钟

**风险**: 🔴 高 - 涉及自动发布，需谨慎测试

---

## 🎯 推荐实施顺序

### 第一阶段（本周）：基础设施

- [ ] **第1步**: 代码规范 (10分钟)
- [ ] **第2步**: Git Hooks (15分钟)
- [ ] **第3步**: 增强CI (20分钟)

### 第二阶段（下周）：可观测性

- [ ] **第4步**: Codecov (15分钟)
- [ ] **第5步**: 完整质量门禁 (30分钟 + 1周观察)

### 第三阶段（2周后）：高级功能

- [ ] **第6步**: 安全和性能 (30分钟)
- [ ] **第7步**: 自动发布 (45分钟)

---

## 📊 进度追踪表

| 步骤  | 状态      | 开始时间 | 完成时间 | 测试结果 | 备注           |
| ----- | --------- | -------- | -------- | -------- | -------------- |
| 第0步 | ✅ 完成   | -        | -        | -        | 配置文件已创建 |
| 第1步 | ⏳ 待开始 | -        | -        | -        | -              |
| 第2步 | ⏳ 待开始 | -        | -        | -        | -              |
| 第3步 | ⏳ 待开始 | -        | -        | -        | -              |
| 第4步 | ⏳ 待开始 | -        | -        | -        | -              |
| 第5步 | ⏳ 待开始 | -        | -        | -        | -              |
| 第6步 | ⏳ 待开始 | -        | -        | -        | -              |
| 第7步 | ⏳ 待开始 | -        | -        | -        | -              |

---

## 🚨 应急预案

### 如果某步失败：

1. **不要慌** - 所有改动都在版本控制中
2. **查看错误** - 分析具体失败原因
3. **尝试修复** - 简单问题可以快速修复
4. **果断回滚** - 复杂问题先回滚再研究
5. **记录问题** - 在本文档记录遇到的问题

### 常见问题排查：

- **Husky不工作**: `chmod +x .husky/*`
- **CI失败**: 检查 GitHub Actions 日志
- **覆盖率不达标**: 使用 `continue-on-error: true` 临时放宽
- **发布失败**: 检查 token 权限和包名

---

## 📝 实施日志

### 第1步实施日志

```
日期: ____
执行人: ____
测试结果:
问题:
解决方案:
```

### 第2步实施日志

```
日期: ____
执行人: ____
测试结果:
问题:
解决方案:
```

（以此类推...）

---

## ✅ 完成标准

每一步完成需满足：

- [ ] 本地测试通过
- [ ] CI测试通过（如适用）
- [ ] 团队成员验证
- [ ] 文档已更新
- [ ] 回滚方案已验证

全部完成标准：

- [ ] 所有7步都已完成
- [ ] 质量指标达标
- [ ] 团队熟悉新流程
- [ ] 无明显性能影响
- [ ] 应急预案已演练

---

**创建日期**: 2025-11-16
**维护者**: @stoa
