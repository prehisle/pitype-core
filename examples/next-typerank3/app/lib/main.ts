...
export function initTyperank3Demo(): void {
  ...
  if (!textContainer || !textDisplay || !resultModal || !restartBtn) {
    console.warn('缺少必要的 DOM 节点，无法初始化 TypeRank3 示例。');
    return;
  }
  ...
  const infoModal = document.getElementById('info-modal') as HTMLElement | null;
  const infoTitle = document.getElementById('info-title') as HTMLElement | null;
  const infoContent = document.getElementById('info-content') as HTMLElement | null;
  const infoCloseBtn = document.getElementById('info-close-btn') as HTMLElement | null;
  ...
  if (infoModal && infoTitle && infoContent && infoCloseBtn) {
    initInfoModal({
