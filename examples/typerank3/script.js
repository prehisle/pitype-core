...
const getLocaleText = typeof window.getText === 'function' ? (key) => window.getText(key) : () => '';
...
function applyTheme(theme) {
    // 移除body上的所有主题类
    document.body.classList.remove('theme-dracula', 'theme-serika', 'theme-botanical', 'theme-aether', 'theme-nord');
...
function renderTokenizedContent(source) {
    ...
    tokens.forEach(token => {
        ...
        const word = token.attachToPrevious && currentWord
            ? currentWord
...
function init() {
    ...
    if (typeof forcedTextIndex === 'number' && !Number.isNaN(forcedTextIndex) && textLibrary[forcedTextIndex]) {
...
function cacheAllCharSpans() {
        ...
        for (let i = 0; i < charSpans.length; i++) {
            ...
            // 普通单词容器，需要添加子元素
                ...
                innerSpans.forEach(innerSpan => {
                    // 过滤掉嵌套在no-break内的字符
                    if (!innerSpan.parentElement.classList.contains('word-space') && 
    ...
    allCharSpans.sort((a, b) => {
        ...
        if (Math.abs(rowDiff) > 5) { // 如果y差值足够大，认为是不同行
            return rowDiff;
        }
        // 同一行按照x位置（从左到右）排序
